...
def question(progress_tracker = None, story_mode = False, qn = None):
    ...
    if qn_no == 1:
        ...
        if story_mode: 
            msg = f'\nYour first class today is science! Your teacher lets you watch a {random_minutes} minutes movie about plants. How many hours is that? (hour:minute format)'
        else:
            msg = f'\nConvert {random_minutes} minutes to hours.\n1 hour = 60 minutes.\nDivide the number of minutes by 60 to get the number of hours. (hour:minute format)'
        ...
        if story_mode:
            random_hours = 72
            msg = f'\nYour friend says you can survive without food for {random_hours} hours. How many days is that?'
        else:
            random_hours = (random.randint(24, 481)// 24) * 24  
            msg = f'\nConvert {random_hours} hours to days.\n1 day = 24 hours.\nDivide the number of hours by 24 to get the number of days.'
